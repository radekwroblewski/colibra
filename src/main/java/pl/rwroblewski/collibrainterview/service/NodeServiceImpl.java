...
public class NodeServiceImpl implements NodeService {
    ...
    @Override
    public String addNode(String nodeName) throws ValidationException {
    ...
    @Override
    public String addEdge(String startNodeName, String endNodeName, int weight) throws ValidationException {
        synchronized (nodeRepository) {
    ...
    @Override
    public String removeNode(String nodeName) throws ValidationException {
        synchronized (nodeRepository) {
    ...
    @Override
    public String removeEdge(String startNodeName, String endNodeName) throws ValidationException {
        synchronized (nodeRepository) {
            nodeValidator.validateExists(startNodeName);
            nodeValidator.validateExists(endNodeName);
    ...
    @Override
    public String shortestPath(String startNodeName, String endNodeName) throws ValidationException {
        synchronized (nodeRepository) {
            nodeValidator.validateExists(startNodeName);
            nodeValidator.validateExists(endNodeName);
    ...
    @Override
    public String closerThan(String nodeName, int weight) throws ValidationException {
        synchronized (nodeRepository) {
            nodeValidator.validateExists(nodeName);
    ...
    private Set<Node> findNodesCloserThan(Node node, int weight, Set<Node> visitedNodes) {
        ...
        if (weight > 0) {
            ...
            nodes.addAll(node.getEdgesFrom().entrySet().stream()
                    .filter(entry -> !visitedNodes.contains(entry.getKey()) && !entry.getValue().isEmpty())
                    .map(entry -> findNodesCloserThan(entry.getKey(), weight - getShortestEdgeWeight(entry),
                            visitedNodes))
